...
def main():
    ...
    # Best method according to macro_f1
    best_method = (
        summary_df[summary_df["metric"] == "macro_f1"]
        .sort_values("mean", ascending=False)
        .iloc[0]["method"]
    ...
    # Load aggregated OOF predictions of the best method
    best_oof_path = Path(OUT_DIR) / best_method / "oof_predictions_aggregated.csv"
    best_oof = pd.read_csv(best_oof_path)

    y_true = best_oof["y_true"].to_numpy()
    y_pred = best_oof["y_pred"].to_numpy()
    ...
    disp = ConfusionMatrixDisplay(
        confusion_matrix=cm,
        display_labels=[LABEL_MAP[0], LABEL_MAP[1], LABEL_MAP[2]],
    )
    disp.plot(ax=ax_cm, cmap="Greys", colorbar=False)
    ax_cm.set_title(f"Confusion Matrix — Best Model ({best_method})")

    # Save figure
    cm_png = Path(OUT_DIR) / f"confusion_matrix_best_{best_method}.png"
    cm_svg = Path(OUT_DIR) / f"confusion_matrix_best_{best_method}.svg"
    fig_cm.savefig(cm_png, dpi=220, bbox_inches="tight")
    fig_cm.savefig(cm_svg, bbox_inches="tight")
    plt.close(fig_cm)

    print(f"✅ Confusion matrix saved at: {cm_png.resolve()}")

    # =========================
    # Minority class analysis (BEST MODEL) — support, errors + 95% CI
    # =========================
    minority_label = MINORITY_CLASS  # e.g., 2 == "High"
    minority_name = LABEL_MAP.get(minority_label, str(minority_label))

    # Load aggregated OOF predictions (1 prediction per instance)
    best_oof_path = Path(OUT_DIR) / best_method / "oof_predictions_aggregated.csv"
    best_oof = pd.read_csv(best_oof_path)
    ...
    # Confusion matrix on aggregated OOF (valid for support/errors)
    labels = [0, 1, 2]
    cm = confusion_matrix(y_true, y_pred, labels=labels)

    k = labels.index(minority_label)
    ...
    support = int((y_true == minority_label).sum())
    pred_as_minority = int((y_pred == minority_label).sum())

    # Pull 95% CI for minority metrics from summary_df (computed over folds)
    def _get_ci(metric_name: str):
        row = summary_df[(summary_df["method"] == best_method) & (summary_df["metric"] == metric_name)]
        if len(row) == 0:
            return (np.nan, np.nan, np.nan)
        r = row.iloc[0]
        return (float(r["mean"]), float(r["ci_lo"]), float(r["ci_hi"]))

    prec_mean, prec_lo, prec_hi = _get_ci("precision_HIGH")
    rec_mean, rec_lo, rec_hi = _get_ci("recall_HIGH")
    f1_mean, f1_lo, f1_hi = _get_ci("f1_HIGH")
    auprc_mean, auprc_lo, auprc_hi = _get_ci("auprc_HIGH")  # optional but useful

    minority_table = pd.DataFrame([
        {"BestMethod": best_method, "MinorityClass": minority_name, "Metric": "Support (true)", "Value": support, "CI95_Lo": "", "CI95_Hi": ""},
        {"BestMethod": best_method, "MinorityClass": minority_name, "Metric": "Predicted as minority", "Value": pred_as_minority, "CI95_Lo": "", "CI95_Hi": ""},
        {"BestMethod": best_method, "MinorityClass": minority_name, "Metric": "TP", "Value": TP, "CI95_Lo": "", "CI95_Hi": ""},
        {"BestMethod": best_method, "MinorityClass": minority_name, "Metric": "FN", "Value": FN, "CI95_Lo": "", "CI95_Hi": ""},
        {"BestMethod": best_method, "MinorityClass": minority_name, "Metric": "FP", "Value": FP, "CI95_Lo": "", "CI95_Hi": ""},
        {"BestMethod": best_method, "MinorityClass": minority_name, "Metric": "TN", "Value": TN, "CI95_Lo": "", "CI95_Hi": ""},
        {"BestMethod": best_method, "MinorityClass": minority_name, "Metric": "Precision", "Value": prec_mean, "CI95_Lo": prec_lo, "CI95_Hi": prec_hi},
        {"BestMethod": best_method, "MinorityClass": minority_name, "Metric": "Recall", "Value": rec_mean, "CI95_Lo": rec_lo, "CI95_Hi": rec_hi},
        {"BestMethod": best_method, "MinorityClass": minority_name, "Metric": "F1", "Value": f1_mean, "CI95_Lo": f1_lo, "CI95_Hi": f1_hi},
        {"BestMethod": best_method, "MinorityClass": minority_name, "Metric": "AUPRC (one-vs-rest)", "Value": auprc_mean, "CI95_Lo": auprc_lo, "CI95_Hi": auprc_hi},
    ])

    out_minority_csv = Path(OUT_DIR) / f"minority_class_analysis_best_{best_method}.csv"
    minority_table.to_csv(out_minority_csv, index=False)

    print("\n📌 Minority class analysis (best model):")
    print(minority_table.to_string(index=False))
    print(f"\n✅ Minority class analysis saved at: {out_minority_csv.resolve()}")

    # =========================
    # FIGURE: Row-normalized confusion matrix (Best Model)
    # =========================

    # y_true, y_pred already loaded from aggregated OOF
    # cm already computed as confusion_matrix(y_true, y_pred, labels=[0,1,2])
    labels = [0, 1, 2]
    label_names = [LABEL_MAP[l] for l in labels]

    # Row-normalized (recall per class): each row sums to 1 (or 0 if empty)
    row_sums = cm.sum(axis=1, keepdims=True)
    cm_norm = np.divide(cm, row_sums, out=np.zeros_like(cm, dtype=float), where=row_sums != 0)

    fig_n, ax_n = plt.subplots(figsize=(6.2, 5.2))

    # Use very light grayscale to avoid dark cells in print
    # (If you prefer color, replace cmap with "Blues" or "YlGnBu")
    im = ax_n.imshow(cm_norm, cmap="Greys", vmin=0.0, vmax=1.0)

    ax_n.set_title(f"Row-normalized Confusion Matrix — Best Model ({best_method})", fontsize=12, pad=10)
    ax_n.set_xlabel("Predicted label", fontsize=11)
    ax_n.set_ylabel("True label", fontsize=11)

    ax_n.set_xticks(range(len(labels)))
    ax_n.set_yticks(range(len(labels)))
    ax_n.set_xticklabels(label_names, rotation=0)
    ax_n.set_yticklabels(label_names)

    # Annotate each cell with: percentage and (count)
    # Percentage is row-normalized: 100 * cm_norm[i,j]
    for i in range(cm.shape[0]):
        for j in range(cm.shape[1]):
            pct = 100.0 * cm_norm[i, j]
            cnt = int(cm[i, j])

            # White text on diagonal (dark cells), black elsewhere
    ...
    # Clean frame; no grid lines
    ax_n.set_aspect("equal")
    for spine in ax_n.spines.values():
        spine.set_visible(True)

    # Optional: add colorbar (often omitted in papers to save space)
    # cbar = fig_n.colorbar(im, ax=ax_n, fraction=0.046, pad=0.04)
    # cbar.set_label("Row-normalized proportion", rotation=90)

    fig_n.tight_layout()

    # Save paper-ready outputs
    out_norm_png = Path(OUT_DIR) / f"confusion_matrix_best_{best_method}_row_norm.png"
    out_norm_svg = Path(OUT_DIR) / f"confusion_matrix_best_{best_method}_row_norm.svg"
    fig_n.savefig(out_norm_png, dpi=300, bbox_inches="tight")
    fig_n.savefig(out_norm_svg, bbox_inches="tight")
    plt.close(fig_n)

    print(f"✅ Row-normalized confusion matrix saved at: {out_norm_png.resolve()}")




    # =========================
    # EXTRA: Confusion Matrix (OOF accumulated / summed)
    # =========================
    best_oof_raw_path = Path(OUT_DIR) / best_method / "oof_predictions_raw.csv"
    best_oof_raw = pd.read_csv(best_oof_raw_path)

    y_true_raw = best_oof_raw["y_true"].to_numpy()
    y_pred_raw = best_oof_raw["y_pred"].to_numpy()

    cm_oof_sum = confusion_matrix(y_true_raw, y_pred_raw, labels=[0, 1, 2])

    fig_cm2, ax_cm2 = plt.subplots(figsize=(5, 4))
    disp2 = ConfusionMatrixDisplay(
        confusion_matrix=cm_oof_sum,
        display_labels=[LABEL_MAP[0], LABEL_MAP[1], LABEL_MAP[2]],
    )
    disp2.plot(ax=ax_cm2, cmap="Greys", colorbar=False)
    ax_cm2.set_title(f"Confusion Matrix — OOF Accumulated ({best_method})")

    cm2_png = Path(OUT_DIR) / f"confusion_matrix_oof_sum_{best_method}.png"
    cm2_svg = Path(OUT_DIR) / f"confusion_matrix_oof_sum_{best_method}.svg"
    fig_cm2.savefig(cm2_png, dpi=220, bbox_inches="tight")
    fig_cm2.savefig(cm2_svg, bbox_inches="tight")
    plt.close(fig_cm2)

    print(f"✅ OOF accumulated confusion matrix saved at: {cm2_png.resolve()}")
